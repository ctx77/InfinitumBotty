...
class ModuleReloader(object):
    ...
    def update_on_priv_msg(self, data: dict, connection: Connection):
        if data["message"].startswith(".module") and self._is_idented_mod(data, connection) and connection.details.get_channel() == "#faust-bot":
            ...
            _active_modules = {str(type(_active_module)): _active_module for _active_module in connection.priv_msg_observable._observers}
            _reloadable_mods = {
            ...
            if _cmd == "remove" or _cmd == "rm":
                if _mod in _reloadable_mods:
                    if _reloadable_mods[_mod] in _active_modules:
                        while _active_modules[_reloadable_mods[_mod]] in connection.priv_msg_observable._observers:
                            connection.priv_msg_observable._observers.remove(_active_modules[_reloadable_mods[_mod]])
                        connection.send_back(
                ...
                if _mod in _reloadable_mods:
                    ...
                    match _mod:
                        ...
                        case "module_reloader":
                            ...
                            _new_mod = reload(_new_mod)
                            connection.priv_msg_observable._observers.append(_new_mod.ModuleReloader())

                        case "comic":
                            import faustbot.modules.ComicObserver as _new_mod
                            _new_mod = reload(_new_mod)
                            connection.priv_msg_observable._observers.append(_new_mod.ComicObserver())
